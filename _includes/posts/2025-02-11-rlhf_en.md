...
Our objective is to maximize the probability that $y_1$ beats $y_2$ across the entire annotated dataset \\(\mathcal{D} = \{x^i, y_w^i, y_l^i \}_{i=1}^N\\). That is, we want the chosen responses to consistently outperform the rejected ones. Accordingly, the overall optimization objective for the reward function can be formulated as:
