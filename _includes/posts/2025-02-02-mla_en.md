...
Thanks to associative properties of matrix multiplication, $\mathbf{W}_q^{(s)} \mathbf{W}_{kc}^{(s)\top}$ can be merged into a single matrix $\mathbf{W}_{\mathrm{merged}}^{(s)}$. Hence, we only keep $\mathbf{c}_i$ in VRAM, never storing all the $\mathbf{k}_i^{(s)}$.  
