...
In a Transformer, a token sequence $\boldsymbol{x}_1,\dots,\boldsymbol{x}_l$ is projected into multiple sets of $(Q, K, V)$ for attention computation. For the $s$-th head, assuming hidden dimensionality $d$:
$$
\boldsymbol{q}_i^{(s)} = \boldsymbol{x}_i \boldsymbol{W}_q^{(s)},\quad
\boldsymbol{k}_i^{(s)} = \boldsymbol{x}_i \boldsymbol{W}_k^{(s)},\quad
...
$$
\alpha_{t,i}^{(s)} = \boldsymbol{q}_t^{(s)} \,\boldsymbol{k}_i^{(s)\top}, \quad\text{for } i \le t.
$$
To speed up inference, we cache the computed $\boldsymbol{k}_i^{(s)}$ and $\boldsymbol{v}_i^{(s)}$ in VRAM for later tokens, a storage referred to as the **KV-Cache**.
...
In **MLA (Multi-Head Latent Attention)**, we still project each $\boldsymbol{x}_i$ into Key and Value at training time, but we introduce a **low-rank latent vector** $\boldsymbol{c}_i$. During inference, instead of caching high-dimensional multi-head K/V, we only store this compact $\boldsymbol{c}_i$, then **merge matrices** when needed to reconstruct the multi-head K/V. Concretely, ignoring RoPE for the moment, we can represent MLA’s training step like so:
$$
\boldsymbol{c}_i = \boldsymbol{x}_i \boldsymbol{W}_c \quad (\text{a low-rank projection, } \boldsymbol{W}_c \in \mathbb{R}^{d \times d_c}),
$$
and for each head $s$ we define projection matrices $\boldsymbol{W}_{kc}^{(s)}, \boldsymbol{W}_v^{(s)}$ such that
$$
\boldsymbol{k}_i^{(s)} = \boldsymbol{c}_i \boldsymbol{W}_{kc}^{(s)}, \quad
\boldsymbol{v}_i^{(s)} = \boldsymbol{c}_i \boldsymbol{W}_v^{(s)}.
$$
As a result, no matter how many heads we have, only training time sees explicit multi-head K/V. At **inference**, we simply cache the latent vector $\boldsymbol{c}_i$, reconstructing K/V on-the-fly using matrix combinations. This is how MLA largely decouples the KV-Cache cost from the number of heads.

### 3.2 On-Demand Decompression: How VRAM is Saved

During inference, whenever we generate token $t$ and need to compute the dot product $\boldsymbol{q}_t^{(s)} \boldsymbol{k}_i^{(s)\top}$ for previous tokens $i < t$, a conventional approach would read $\boldsymbol{k}_i^{(s)}$ from VRAM. MLA, however, merges them:

$$
\boldsymbol{k}_i^{(s)} = \boldsymbol{c}_i\,\boldsymbol{W}_{kc}^{(s)}\quad\Longrightarrow\quad\boldsymbol{q}_t^{(s)} \boldsymbol{k}_i^{(s)\top}=(\boldsymbol{x}_t \boldsymbol{W}_q^{(s)})(\boldsymbol{c}_i \boldsymbol{W}_{kc}^{(s)})^\top.
$$

Thanks to associative properties of matrix multiplication, $\boldsymbol{W}_q^{(s)} \boldsymbol{W}_{kc}^{(s)\top}$ can be merged into a single matrix $\boldsymbol{W}_{\mathrm{merged}}^{(s)}$. Hence, we only keep $\boldsymbol{c}_i$ in VRAM, never storing all the $\boldsymbol{k}_i^{(s)}$.  
If $\boldsymbol{c}_i$ is $d_c$-dimensional, with $d_c \ll h \times d_k$, then the KV-Cache cost transitions from $h \times d_k$ down to $d_c$.

### 3.3 How Low-Rank Projection Drastically Reduces Storage

One might ask, how big can the compression ratio get? Suppose $d=4096$, $h=32$, and single-head dimension $d_k=128$. In standard MHA, each token’s Key is $32 \times 128 = 4096$ elements (similarly for Value). MLA might set the latent vector $\boldsymbol{c}_i$ to 512 elements, cutting VRAM usage from 4096 to 512—an 8× improvement. In more extreme setups, you might see tens or even hundreds of times in compression factor.
...
Imagine every time you snap a photo (process a token $\boldsymbol{x}_i$), instead of saving the “full-resolution image plus all filters,” you only keep a **“small-yet-informative thumbnail”**—the latent vector $\boldsymbol{c}_i$. For instance, if the original image resolution is 4096 (like $d=4096$ in MLA), the thumbnail might be 512 in size, achieving roughly 1/8 the original.  
Mathematically,
$$
\boldsymbol{c}_i = \boldsymbol{x}_i \boldsymbol{W}_c, \quad
\boldsymbol{W}_c \in \mathbb{R}^{4096 \times 512}.
$$
...
\boldsymbol{v}_i^{(s)} = \boldsymbol{c}_i \boldsymbol{W}_v^{(s)}.
$$
So no matter how many filters (heads) exist, you only keep the thumbnail ($\boldsymbol{c}_i$) rather than multiple versions of the same image. At inference, each filter’s parameters can reconstruct the Key or Value from that thumbnail, resulting in **massive storage reduction**.
...
$$
\boldsymbol{q}_t^{(s)} \boldsymbol{k}_i^{(s)\top} = (\boldsymbol{x}_t \boldsymbol{W}_q^{(s)})(\boldsymbol{c}_i \boldsymbol{W}_{kc}^{(s)})^\top \approx \boldsymbol{x}_t \boldsymbol{W}_{\mathrm{merged}}^{(s)} \boldsymbol{c}_i^\top.
$$

Hence, the filter parameters $\boldsymbol{W}_{kc}^{(s)}$ do not have to be stored in the “album” for each picture (head). Only the compact thumbnail $\boldsymbol{c}_i$ remains. This approach **omits** the repeated Key/Value overhead that standard multi-head attention requires.

---

## 5. The RoPE Challenge: Why Add a “Position Sticker”?

Now that we’ve covered MLA’s low-rank approach and on-demand reconstruction, we must address a practical reality. Transformers often rely on **RoPE** (Rotary Position Embedding) to incorporate positional information into Key/Query. RoPE complicates the straightforward “latent vector only” approach, since each token’s position $i$ introduces a rotation matrix $\boldsymbol{\mathcal{R}}_i$ that affects the dot product differently.

### 5.1 RoPE: Timestamp and GPS Coordinates

Returning to our album analogy, RoPE becomes the **“timestamp or GPS location”** on each photo. Besides the core visual content ($\boldsymbol{c}_i$), we retain a small sticker that encodes when and where the photo was taken. If we tried to embed the time data directly into the thumbnail, relative distances (time differences) might be lost. Thus, in MLA, a portion of K/Query dimension remains explicitly multiplied by $\boldsymbol{\mathcal{R}}_i$, preserving relative position even under our low-rank scheme.

### 5.2 Split Strategy: $\boldsymbol{c}_i$ + a Small RoPE Dimension

In formal terms, to preserve the rotation property $\boldsymbol{\mathcal{R}}_m \boldsymbol{\mathcal{R}}_n^\top = \boldsymbol{\mathcal{R}}_{m-n}$, MLA splits each Key (and similarly Query) into two parts:

$$
\boldsymbol{k}_i^{(s)}=\underbrace{\bigl(\boldsymbol{c}_i \boldsymbol{W}_{kc}^{(s)}\bigr)}_{\text{compressed portion}}\;\oplus\;\underbrace{\bigl(\boldsymbol{x}_i \boldsymbol{W}_{kr}\,\boldsymbol{\mathcal{R}}_i\bigr)}_{\text{positional portion}},
$$

so the KV-Cache only grows by a modest “position dimension,” while the main storage remains the latent vector $\boldsymbol{c}_i$. This design deftly fuses **low-rank projection** with **rotary embeddings** for relative positions.
...
- **Storage**: Traditional multi-head attention must store K/V for every head. MLA instead uses a latent vector $\boldsymbol{c}_i$ (low-rank) plus a small dimension for RoPE if needed. VRAM can shrink by factors of several times or more.  
...
When splitting K into a low-rank zone ($\boldsymbol{c}_i$) vs. RoPE zone, if the RoPE dimension is too small, extremely long contexts may not get enough positional signal. Conversely, if it’s too large, MLA’s compression advantages diminish. The optimal trade-off typically emerges from empirical experiments.

### 7.3 Numerical Stability and Precision

Because MLA merges weight matrices (e.g., $\boldsymbol{W}_q^{(s)} \boldsymbol{W}_{kc}^{(s)\top}$) at inference time, using BF16/FP16 can introduce small accumulative errors from changing multiply orders. Generally, it’s acceptable. If your application is ultra-sensitive to accuracy, consider higher precision accumulators or partial float32 fallback.

---

## 8. Overall Summary and Future Directions

**MLA (Multi-Head Latent Attention)** is more than merely “low-rank factorization of K/V.” It pushes further by **caching only a latent vector** $\boldsymbol{c}_i$ in **inference**, reconstructing multi-head K/V via **on-demand decompression** and **matrix merging**—cutting KV-Cache usage drastically. Then, by applying a **split strategy** for RoPE, MLA retains relative positional information without forcing the entire K/V to remain explicit.
...
$$
\boldsymbol{c}_i = \boldsymbol{x}_i \boldsymbol{W}_c 
...
$$
\boldsymbol{k}_i^{(s)} = \boldsymbol{c}_i \boldsymbol{W}_{kc}^{(s)}, 
\quad
\boldsymbol{v}_i^{(s)} = \boldsymbol{c}_i \boldsymbol{W}_v^{(s)}
\quad\Longleftrightarrow\quad
...
$$
\boldsymbol{q}_t^{(s)} \boldsymbol{k}_i^{(s)\top}
=
(\boldsymbol{x}_t \boldsymbol{W}_q^{(s)})
(\boldsymbol{c}_i \boldsymbol{W}_{kc}^{(s)})^\top
\quad\Longleftrightarrow\quad
...
\;\oplus\;
(\boldsymbol{x}_i \boldsymbol{W}_{kr}\boldsymbol{\mathcal{R}}_i)
\quad\Longleftrightarrow\quad
